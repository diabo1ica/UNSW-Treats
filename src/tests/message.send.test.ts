import { requestClear, requestRegister, requestChannelsCreate, requestMessageSend, generateMessage } from './request';
...
describe('Test suite for /message/send/v1', () => {
  ...
  test('route success', () => {
    const messageObj = requestMessageSend(usertoken1, channelId1, 'GOODMorining');
  ...
  test('invalid channelId (400 error)', () => {
    const messageObj = requestMessageSend(usertoken1, -1, 'Helloooo!!!!!');
  ...
  test('message.length > 1000 (400 error)', () => {
    ...
    expect(requestMessageSend(usertoken1, channelId1, message).statusCode).toStrictEqual(INPUT_ERROR);
  });
  test('message.length < 1 (400 error)', () => {
    expect(requestMessageSend(usertoken1, channelId1, '').statusCode).toStrictEqual(INPUT_ERROR);
  });

  test('token/user is not a member of channel (403 error)', () => {
    expect(requestMessageSend(usertoken2, channelId1, 'Hi, how are you?').statusCode).toStrictEqual(AUTHORISATION_ERROR);
  });
});
